...
| Function Code | Name | Purpose | Address Format |
|---------------|------|---------|----------------|
| **0x01** | Read Coils | Read relay states | Coil-based (00001-00032) |
| **0x03** | Read Holding Registers | Read relay states & config | Register-based (40001-40256) |
| **0x05** | Write Single Coil | Control single relay | Coil-based (00001-00008) |
| **0x06** | Write Single Register | Control single relay & config | Register-based (40001-40256) |
| **0x0F (15)** | Write Multiple Coils | Control multiple relays | Coil-based (00001-00008) |
| **0x10 (16)** | Write Multiple Registers | Control multiple relays | Register-based (40001-40256) |
...
|--------------|------|-------------|-----------------|
| 0x0100 | OPEN | Turn relay ON (energize) | 0x00 |
| 0x0200 | CLOSE | Turn relay OFF (de-energize) | 0x00 |
...
| 0x0700 | OPEN_ALL | Turn all relays ON (addr 0x0000 only) | 0x00 |
| 0x0800 | CLOSE_ALL | Turn all relays OFF (addr 0x0000 only) | 0x00 |
...
# R1: OPEN, R2: MOMENTARY, R3: DELAY 5s, R4: TOGGLE, R5-8: CLOSE
...
**Example:**
```cpp
// 1. Send OPEN command (0x0100)
ryn4.controlRelay(1, RelayAction::OPEN);
...
**Example: Open Channel 1 (Slave ID = 0x01)**
```
01 06 00 01 01 00 D9 9A
│  │  │  │  │  │  │  └─ CRC Hi
│  │  │  │  │  │  └──── CRC Lo
│  │  │  │  │  └─────── Delay = 0x00
│  │  │  │  └────────── Command = 0x01 (OPEN, full value 0x0100)
│  │  │  └───────────── Address Lo = 0x01
│  │  └──────────────── Address Hi = 0x00
│  └─────────────────── Function = 0x06
└────────────────────── Slave ID = 0x01
...
```
Channel 1 OPEN (turn ON):
  01 06 00 01 01 00 D9 9A

Channel 1 CLOSE (turn OFF):
...
- Remember: Write 0x0100 (OPEN), but read back 0x0001
