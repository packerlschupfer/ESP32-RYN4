 ...
 * - Write commands use 0x0100 (OPEN) / 0x0200 (CLOSE)
...
namespace hardware {
     ...
     * Special address: 0x0000 for OPEN_ALL/CLOSE_ALL broadcast commands
     */
     ...
     *
     * Write: 0x0100 (OPEN command) → Read back: 0x0001 (ON status)
     * Write: 0x0200 (CLOSE command) → Read back: 0x0000 (OFF status)
     */
    static constexpr uint16_t CMD_OPEN = 0x0100;      ///< Turn relay ON (energize)
    static constexpr uint16_t CMD_CLOSE = 0x0200;     ///< Turn relay OFF (de-energize)
    ...
    static constexpr uint16_t CMD_DELAY_BASE = 0x0600;///< Delay base (OR with seconds)
    static constexpr uint16_t CMD_OPEN_ALL = 0x0700;  ///< Turn all relays ON (addr 0x0000 only)
    static constexpr uint16_t CMD_CLOSE_ALL = 0x0800; ///< Turn all relays OFF (addr 0x0000 only)
     ...
     * - Send CMD_OPEN (0x0100) → Read back STATUS_ON (0x0001)
     * - Send CMD_CLOSE (0x0200) → Read back STATUS_OFF (0x0000)
     *
    ...
    inline constexpr uint16_t boolToCommand(bool state) {
        return state ? CMD_OPEN : CMD_CLOSE;
    }
     ...
     *    Write: 0x0100 (OPEN), 0x0200 (CLOSE)
     ...
     *    Broadcast:  0x0000 (OPEN_ALL/CLOSE_ALL only)
     *
     * 4. COMMAND MODES:
     *    - OPEN/CLOSE: Basic ON/OFF control
     ...
     *    - Baud rate: Jumper pads M1/M2 (2400/4800/9600/19200)
