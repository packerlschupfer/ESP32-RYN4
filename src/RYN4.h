/*
 * RYN4.h - part of the ESP32-RYN4 library
 *
...
public:
    ...
    struct RelayCommandSpec {
        ryn4::RelayAction action;  ///< Command to execute (ON, OFF, TOGGLE, LATCH, MOMENTARY, DELAY)
        uint8_t delaySeconds;      ///< Delay duration (used only for DELAY action, 0-255 seconds)

        // Constructors for convenience
        RelayCommandSpec() : action(ryn4::RelayAction::OFF), delaySeconds(0) {}
     ...
     *     {RelayAction::ON, 0},         // Relay 1: Pump ON permanently
    ...
    /**
     * @brief Turn a relay ON (DELAY-safe)
     *
     * Cancels any active DELAY timer, then turns relay ON permanently.
     * Equivalent to forceOnRelay() - use this for guaranteed ON.
     *
     * @note This method always works, even with active DELAY timers.
     * @note For simple ON without DELAY cancellation, use turnOnRelayDirect().
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode turnOnRelay(uint8_t relayIndex);

    /**
     * @brief Turn a relay OFF (DELAY-safe)
     *
     * Sends DELAY 0 to cancel any active timer and turn relay OFF immediately.
     * Equivalent to forceOffRelay() - use this for guaranteed OFF.
     *
     * @note This method always works, even with active DELAY timers.
     * @note For simple OFF without DELAY cancellation, use turnOffRelayDirect().
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode turnOffRelay(uint8_t relayIndex);

    /**
     * @brief Toggle a relay state (DELAY-safe)
     *
     * Reads current state, cancels any DELAY, sets opposite state.
     * Always works regardless of active DELAY timers.
     *
     * @note This is more expensive than toggleRelayDirect() (1 read + 1-2 writes)
     * @note For simple toggle without DELAY handling, use toggleRelayDirect().
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode toggleRelay(uint8_t relayIndex);

    /**
     * @brief Turn relay ON with hardware watchdog timer
     *
     * Sends DELAY X command: relay turns ON immediately, hardware automatically
     * turns it OFF after specified seconds. Use for safety-critical applications
     * where relay must turn OFF if software stops refreshing.
     *
     * Call periodically to refresh the watchdog (e.g., every 5s with 10s timer).
     *
     * @note DELAY has priority - other commands won't affect relay until timer expires
     * @note To cancel: use turnOffRelay() or send DELAY 0
     *
     * @param relayIndex Relay number (1-8)
     * @param seconds Watchdog timeout (1-255 seconds)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode turnOnRelayTimed(uint8_t relayIndex, uint8_t seconds);

    /**
     * @brief Turn ALL relays ON with hardware watchdog timer
     *
     * Sends DELAY X to all 8 relays atomically using FC 0x10.
     * Use for batch watchdog refresh in safety-critical applications.
     *
     * @note NO automatic refresh - user must call periodically
     * @note If application hangs, hardware turns OFF all relays (safety feature!)
     *
     * Example (boiler controller):
     * @code
     * void safetyLoop() {
     *     while (running) {
     *         checkSafety();              // Application logic
     *         ryn4.turnOnAllTimed(10);    // Refresh watchdog
     *         vTaskDelay(pdMS_TO_TICKS(5000)); // Check every 5s
     *     }
     * }
     * // If loop hangs, hardware turns OFF after 10s (SAFETY!)
     * @endcode
     *
     * @param seconds Watchdog timeout (1-255 seconds)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode turnOnAllTimed(uint8_t seconds);

    /**
     * @brief Send momentary pulse to relay
     *
     * Sends MOMENTARY command (0x0500): relay turns ON for ~1 second, then OFF.
     * Useful for trigger outputs, buzzers, or pulse-based control.
     *
     * @note MOMENTARY can cancel active DELAY timers (hardware behavior)
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode momentaryRelay(uint8_t relayIndex);

    // ========== DIRECT Relay Control Methods (fast, may fail with active DELAY) ==========

    /**
     * @brief Turn relay ON directly (fast, not DELAY-safe)
     *
     * Sends simple ON command (0x0100). Fast but may fail if DELAY is active:
     * - If no DELAY active: relay turns ON immediately
     * - If DELAY active: relay turns ON but timer keeps running, will turn OFF when timer expires!
     *
     * @note Use turnOnRelay() for guaranteed behavior with DELAY timers.
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode turnOnRelayDirect(uint8_t relayIndex);

    /**
     * @brief Turn relay OFF directly (fast, not DELAY-safe)
     *
     * Sends simple OFF command (0x0200). Fast but IGNORED if DELAY is active!
     * - If no DELAY active: relay turns OFF immediately
     * - If DELAY active: command is SILENTLY IGNORED, relay stays ON!
     *
     * @warning This command FAILS SILENTLY with active DELAY timers!
     * @note Use turnOffRelay() for guaranteed behavior.
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent (even if ignored by hardware!)
     */
    ryn4::RelayErrorCode turnOffRelayDirect(uint8_t relayIndex);

    /**
     * @brief Toggle relay directly (fast, not DELAY-safe)
     *
     * Sends simple TOGGLE command (0x0300). Fast but IGNORED if DELAY is active!
     * - If no DELAY active: relay toggles state
     * - If DELAY active: command is SILENTLY IGNORED!
     *
     * @warning This command FAILS SILENTLY with active DELAY timers!
     * @note Use toggleRelay() for guaranteed behavior.
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent (even if ignored by hardware!)
     */
    ryn4::RelayErrorCode toggleRelayDirect(uint8_t relayIndex);
    ...
    /**
     * @brief Force relay OFF and cancel any active DELAY timer
     *
     * Sends DELAY 0 (0x0600) command which:
     * - Turns relay OFF immediately
     * - Cancels any running delay timer
     *
     * Use this instead of OFF when you need to ensure the relay
     * turns off immediately, even if a DELAY command is active.
     *
     * @note Regular OFF (0x0200) does NOT cancel active delays!
     * @note Hardware verified 2025-12-14
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode forceOffRelay(uint8_t relayIndex);

    /**
     * @brief Force relay ON and cancel any active DELAY timer
     *
     * Sends DELAY 0 (0x0600) to cancel any active timer, then ON (0x0100):
     * - Cancels any running delay timer
     * - Turns relay ON immediately and permanently
     *
     * Use this to guarantee the relay turns ON even if a DELAY command
     * is currently active and about to turn the relay OFF.
     *
     * @note Hardware verified 2025-12-14
     *
     * @param relayIndex Relay number (1-8)
     * @return RelayErrorCode SUCCESS if both commands sent
     */
    ryn4::RelayErrorCode forceOnRelay(uint8_t relayIndex);

    /**
     * @brief Emergency stop - force all relays OFF and cancel all delays
     *
     * Sends DELAY 0 (0x0600) to all 8 relays using FC 0x10 (Write Multiple Registers).
     * Guarantees all relays turn OFF immediately, even if DELAY timers are active.
     *
     * This is the safest way to turn off all relays as it cancels any active
     * delay timers that would otherwise keep relays ON.
     *
     * @note Regular OFF/ALL_OFF does NOT cancel active delays!
     * @note Hardware verified 2025-12-14
     *
     * @return RelayErrorCode SUCCESS if command sent
     */
    ryn4::RelayErrorCode emergencyStopAll();
     ...
     * 
     * @param relayIndex Relay number (1-8)
     * @param action Action to perform (ON, OFF, TOGGLE, etc.)
