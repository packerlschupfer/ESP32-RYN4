 ...
 * - Commands: 0x0100 = OPEN (ON), 0x0200 = CLOSE (OFF)
    ...
    if (address <= 0x0007) {
        ...
        if (length >= 2) {
            ...
            if (lock) {
                ...
                if (echoValue == 0x0100) expectedState = true;   // OPEN
                else if (echoValue == 0x0200) expectedState = false; // CLOSE
...
ryn4::RelayErrorCode RYN4::readRelayStatus(uint8_t relayIndex, bool& state) {
    ...
    auto result = readHoldingRegisters(registerAddr, 1);
