...
- **ALL commands work in FC 0x10**: ON, OFF, TOGGLE, LATCH, MOMENTARY, DELAY
...
```cpp
// SAFE methods (DELAY-aware, always work - recommended!)
ryn4.turnOnRelay(1);               // Cancels DELAY + ON permanently
ryn4.turnOffRelay(1);              // DELAY 0 - guaranteed OFF
ryn4.turnOnRelayTimed(1, 10);      // ON with 10s hardware watchdog
ryn4.turnOnAllTimed(10);           // All ON with 10s watchdog
ryn4.momentaryRelay(1);            // 1-second pulse
ryn4.emergencyStopAll();           // All OFF, cancel all timers

// DIRECT methods (fast, but fail silently with active DELAY!)
ryn4.turnOnRelayDirect(1);         // ON (0x0100) - timer not cancelled
ryn4.turnOffRelayDirect(1);        // OFF (0x0200) - IGNORED if DELAY active!

// Reading status
bool state;
ryn4.readRelayStatus(1, state);    // Receives 0x0001 (ON) or 0x0000 (OFF)
```

This asymmetry is a hardware protocol characteristic. The library handles conversion automatically.

## ⚠️ CRITICAL: Understanding DELAY Commands

The RYN4 hardware implements DELAY commands as **hardware watchdog timers with absolute priority**.

### Why DELAY Has Priority

DELAY is a **safety feature**: if your software crashes, the hardware automatically turns OFF the relay after the timer expires. To prevent bugs from accidentally canceling this safety, the hardware **ignores** regular ON/OFF commands while DELAY is active.

### What Works with Active DELAY?

| Command | Works? | Behavior |
|---------|--------|----------|
| `turnOffRelay()` | ✅ YES | Sends DELAY 0, cancels timer |
| `turnOnRelay()` | ✅ YES | Sends DELAY 0 + ON |
| `turnOffRelayDirect()` | ❌ NO | **SILENTLY IGNORED!** |
| `turnOnRelayDirect()` | ⚠️ Partial | ON but timer keeps running |
| `momentaryRelay()` | ✅ YES | Cancels DELAY (hardware feature) |
| `emergencyStopAll()` | ✅ YES | DELAY 0 × 8 |

### Hardware Watchdog Pattern (Boiler Controller)

```cpp
void safetyLoop() {
    while (running) {
        checkSafetyConditions();       // Your application logic
        ryn4.turnOnAllTimed(10);       // Refresh 10-second watchdog
        vTaskDelay(pdMS_TO_TICKS(5000)); // Check every 5 seconds
    }
}
// If loop hangs, hardware turns OFF all relays after 10s (SAFETY!)
```
...
ryn4->controlRelay(1, RelayAction::OFF);
vTaskDelay(pdMS_TO_TICKS(100));
bool state;
ryn4->readRelayStatus(1, state);

// New approach - automatic verification
auto result = ryn4->controlRelayVerified(1, RelayAction::OFF);
