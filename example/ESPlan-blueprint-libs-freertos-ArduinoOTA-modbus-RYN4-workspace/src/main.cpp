// main.cpp
// 
// RYN4 Example Application using ModbusDevice Architecture
// 
...
void setup() {
    ...
    // Check if watchdog is already initialized to avoid double initialization
    if (esp_task_wdt_status(NULL) == ESP_ERR_INVALID_STATE) {
        // Watchdog not initialized, initialize it
        if (taskManager.initWatchdog(15, true)) {  // 15 second timeout, panic enabled for debugging
            LOG_INFO(LOG_TAG_MAIN, "TaskManager watchdog initialized successfully (15s timeout)");
        } else {
            LOG_ERROR(LOG_TAG_MAIN, "Failed to initialize TaskManager watchdog - continuing without it");
        }
    } else {
        // Watchdog already initialized by another component
        LOG_INFO(LOG_TAG_MAIN, "Watchdog already initialized by another component - skipping init");
        // TaskManager can still use the existing watchdog
    }
...
bool setupModbus() {
    ...
    LOG_INFO(LOG_TAG_MAIN, "Modbus master started with callbacks registered");
    
    // Small delay to ensure Modbus is ready
    delay(50);
    ...
    // Initialize the RYN4 (synchronous initialization in new architecture)
    LOG_INFO(LOG_TAG_MAIN, "Starting RYN4 initialization...");
    ...
    LOG_INFO(LOG_TAG_MAIN, "Waiting for RYN4 initialization to complete (timeout: %dms)...", 
    ...
    // Small delay to let processing task start
    vTaskDelay(pdMS_TO_TICKS(100));
    
    // Queued mode is already enabled during RYN4 initialization
    // No need to enable it again
    LOG_INFO(LOG_TAG_MAIN, "RYN4 queued mode already enabled during initialization");
...
void loop() {
    // Single watchdog registration attempt on first loop iteration
    static bool watchdogAttempted = false;
    if (!watchdogAttempted) {
        watchdogAttempted = true;
        
        // Use safe watchdog registration helper
        if (WatchdogHelper::isCurrentTaskRegistered()) {
            LOG_INFO(LOG_TAG_MAIN, "Loop task already registered with watchdog");
            g_hasWatchdog = true;
        } else {
            // Not registered, try TaskManager first
            TaskManager::WatchdogConfig wdtConfig = TaskManager::WatchdogConfig::enabled(true, 30000);
            LOG_INFO(LOG_TAG_MAIN, "Attempting TaskManager watchdog registration...");
            g_hasWatchdog = taskManager.registerCurrentTaskWithWatchdog("loopTask", wdtConfig);
            
            if (!g_hasWatchdog) {
                LOG_WARN(LOG_TAG_MAIN, "TaskManager registration failed, trying ESP-IDF...");
                // Use safe helper to prevent duplicate registration errors
                g_hasWatchdog = WatchdogHelper::safeAddCurrentTask();
                if (g_hasWatchdog) {
                    LOG_INFO(LOG_TAG_MAIN, "ESP-IDF watchdog registration successful");
                } else {
                    LOG_ERROR(LOG_TAG_MAIN, "ESP-IDF watchdog registration failed");
                }
            }
        }
        
        LOG_INFO(LOG_TAG_MAIN, "Loop watchdog %s", g_hasWatchdog ? "enabled" : "failed");
        
        // Initial feed after registration
        if (g_hasWatchdog) {
            WatchdogHelper::feedWatchdog();
        }
    }
    
    // Periodic maintenance
    static TickType_t lastMaintenanceTick = 0;
    TickType_t currentTick = xTaskGetTickCount();
    
    if ((currentTick - lastMaintenanceTick) >= pdMS_TO_TICKS(1000)) {
        // Feed watchdog if registered
        if (g_hasWatchdog) {
            WatchdogHelper::feedWatchdog();
        }
    ...
    // This will yield to the FreeRTOS scheduler
