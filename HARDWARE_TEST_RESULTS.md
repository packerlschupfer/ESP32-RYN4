...
### TEST 1: Baseline - OPEN/CLOSE ✅

**Command**: FC 0x10, relays 1-4, all OPEN (0x0100)
...
**Command**: FC 0x10, relays 1,3,5,7 MOMENTARY (0x0500), relays 2,4,6,8 CLOSE (0x0200)
...
#  OPEN   MOMT   DLY5s  TOGGL  CLOSE CLOSE CLOSE CLOSE
```

**Results Timeline:**

| Time | R1 (OPEN) | R2 (MOMENTARY) | R3 (DELAY 5s) | R4 (TOGGLE) | R5-8 (CLOSE) |
|------|-----------|----------------|---------------|-------------|--------------|
...
**✅ PASS**:
- Relay 1 (OPEN): Stayed ON permanently ✓
...
|--------------|------------------|---------------------|----------|
| **OPEN** (0x0100) | ✅ Yes | ✅ **YES** | Hardware tested |
| **CLOSE** (0x0200) | ✅ Yes | ✅ **YES** | Hardware tested |
...
The library's `setMultipleRelayStates()` only supports OPEN/CLOSE:
...
struct RelayCommandSpec {
    ryn4::RelayAction action;  // OPEN, CLOSE, TOGGLE, LATCH, MOMENTARY, DELAY
...
```cpp
ryn4.controlRelay(1, RelayAction::OPEN);      // ~50ms
 ...
 *
 * HARDWARE TESTED: All command types (OPEN/CLOSE/TOGGLE/LATCH/MOMENTARY/DELAY)
    ...
    for (size_t i = 0; i < NUM_RELAYS; i++) {
        switch (commands[i].action) {
            ...
            case RelayAction::DELAY:
                ...
                break;
            default:
                data[i] = 0x0200;  // Default to CLOSE
                break;
        }
...
- Mix OPEN/CLOSE/TOGGLE in single command
...
1. ✅ **All command types work in FC 0x10** (not just OPEN/CLOSE)
