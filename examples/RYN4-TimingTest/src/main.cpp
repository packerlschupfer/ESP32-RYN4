...
//
// This example demonstrates the ModbusDevice architecture where:
// 1. Global ModbusRTU instance is set via ModbusRegistry singleton before device creation
// 2. Devices are registered in ModbusRegistry for automatic response routing
...
// 1. Initialize Serial1 for RS485 (8E1 parity)
// 2. Call modbus::ModbusRegistry::getInstance().setModbusRTU(&modbusMaster)
// 3. Setup modbusMaster callbacks (mainHandleData, handleError)
// 4. Create RYN4 instance
// 5. Register device with ModbusRegistry::getInstance().registerDevice()
...
// Note: Now using ryn4::DEFAULT_HARDWARE_CONFIG from the library instead of custom configurations
// The new API uses constexpr arrays in flash memory for hardware config
...
bool setupModbus() {
    ...
    LOG_INFO(LOG_TAG_MAIN, "ModbusRegistry setModbusRTU completed");
    ...
    // Apply default hardware configuration (now using constexpr array in flash)
    relayController->setHardwareConfig(ryn4::DEFAULT_HARDWARE_CONFIG.data());
    LOG_INFO(LOG_TAG_MAIN, "Hardware config applied");
...
void loop() {
    ...
    if (millis() - lastWatchdogFeed >= 1000) {
        ...
        if (!taskManager.feedWatchdog()) { LOG_WARN(LOG_TAG_MAIN, "Watchdog feed failed"); }
        lastWatchdogFeed = millis();
    }
    ...
    if (!minimalTestDone && relayController && relayController->isInitialized() && 
        ...
        if (millis() - minimalTestTime >= 5000 && millis() - minimalTestTime < 8000) {
            ...
            relayController->controlRelay(1, ryn4::RelayAction::OFF);
            ...
            relayController->controlRelay(1, ryn4::RelayAction::ON);
            minimalTestTime = millis() + 3000; // Skip to next phase
        }
            ...
            case 1: // Turn single relay ON
                {
                    ...
                    auto result = relayController->controlRelay(testRelayNum, ryn4::RelayAction::OFF);
                    uint32_t heapAfterControl = ESP.getFreeHeap();
                    
                    if (result == ryn4::RelayErrorCode::SUCCESS) {
                        LOG_INFO(LOG_TAG_MAIN, "✓ Command sent successfully (OFF = relay de-energized)");
                    } else {
            ...
            case 3: // Turn relay OFF
                {
                    ...
                    // Memory tracking - before control command
                    uint32_t heapBeforeControl = ESP.getFreeHeap();
                    auto result = relayController->controlRelay(testRelayNum, ryn4::RelayAction::ON);
                    uint32_t heapAfterControl = ESP.getFreeHeap();
                ...
                if (millis() - singleRelayTestStartTime >= 3000) {  // Increased wait between relay tests
                    if (testRelayNum == 1) {
                        ...
                        for (int i = 1; i <= RYN4_NUM_RELAYS; i++) {
                            relayController->controlRelay(i, ryn4::RelayAction::OFF);
                        }
        ...
        switch (testStep) {
            case 0: // Turn relay ON
                if (relayIndex <= RYN4_NUM_RELAYS) {
                    ...
                    auto resultOn = relayController->controlRelay(relayIndex, ryn4::RelayAction::OFF);
            ...
            case 1: // Wait 2 seconds with relay ON
                if (millis() - relayTestStartTime >= 2000) {
                    ...
                    auto resultOff = relayController->controlRelay(relayIndex, ryn4::RelayAction::ON);
            ...
            case 4: // Wait 3 seconds with pattern
                if (millis() - relayTestStartTime >= 3000) {
                    ...
                    for (int i = 1; i <= RYN4_NUM_RELAYS; i++) {
                        relayController->controlRelay(i, ryn4::RelayAction::ON);
                        relayTestOperations++;
                    }
