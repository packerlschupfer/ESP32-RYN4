...
void RelayControlTask::taskFunction(void* pvParameters) {
    ...
    if (!taskManager.feedWatchdog()) { LOG_WARN(TASK_TAG, "Watchdog feed failed"); }
...
bool RelayControlTask::processSingleRelay(uint8_t relayIndex, bool state) {
    ...
    ryn4::RelayAction action = state ? ryn4::RelayAction::ON : ryn4::RelayAction::OFF;
    ...
    if (result == ryn4::RelayErrorCode::SUCCESS) {
        ...
        LOG_INFO(LOG_TAG_RELAY, "[CMD] R%d: %s", relayIndex, state ? "ON" : "OFF");
        #endif
...
bool RelayControlTask::processSetMultipleRelays(const std::vector<bool>& states) {
    ...
    // Convert vector to std::array<bool, 8> for the new API
    std::array<bool, 8> statesArray = {false, false, false, false, false, false, false, false};
    for (size_t i = 0; i < states.size() && i < 8; i++) {
        statesArray[i] = states[i];
    }

    ryn4::RelayErrorCode result = ryn4Device->setMultipleRelayStates(statesArray);
